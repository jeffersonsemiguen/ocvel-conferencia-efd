---
name: "source-command-core-memory"
description: "Save valuable insights from the current session to storage"
---

# source-command-core-memory

Use this skill when the user asks to run the migrated source command `core-memory`.

## Command Template

# Memory Command

Save session insights to `.Codex/storage/` for future reference.
...
3. **Saves** to `.Codex/storage/memory-{YYYY-MM-DD}.md`
...
Creates: `.Codex/storage/memory-{YYYY-MM-DD}.md`
...
3. Write to storage:
   - Create .Codex/storage/ if not exists
...
Saved to: .Codex/storage/memory-2026-01-23.md
